  ...
  return (
    <section className="labs-panel phrase-studio" aria-labelledby="phrase-studio-title">
      ...
      <div className="labs-control-grid">
        <label className="labs-field"><span>Personality</span><select value={personality} onChange={(event) => { const next = event.target.value as CoachPersonalityId; setPersonality(next); applyTuning(next, delivery, intent); }}>{Object.values(COACH_PERSONALITIES).filter((item) => item.id !== 'drill').map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
